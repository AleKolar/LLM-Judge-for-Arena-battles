...
- **Выбор модели-судьи** из отдельного пула (GPT-4o-mini, DeepSeek, Llama 3.1); по умолчанию DeepSeek
- Автоматическое сравнение ответов и аргументированный вердикт от выбранного судьи
- Поддержка ничьей (DRAW), если решения равнозначны
- При ошибке одной из моделей побеждает успешно ответившая (с указанием причины)
- Сохранение результата битвы в базе данных (модель `ArenaResult`) с информацией о выбранном судье
- Просмотр истории последних 10 битв через API
- Возможность повторного вызова судьи для конкретной битвы по её ID (можно сменить судью)
...
- Файл содержит:
  ...
  - 🧑‍⚖️ Какая модель выступала судьёй
  - ⚖️ Вердикт судьи
...
### 🔄 Расширение списка моделей и судей

- **Модели для битв** добавляются в словарь `AVAILABLE_MODELS` и выпадающий список в `index.html`
- **Модели-судьи** хранятся в отдельном словаре `JUDGE_MODEL`; при добавлении новой записи она автоматически появляется в интерфейсе
- Все идентификаторы моделей OpenRouter поддерживаются; бесплатные модели должны заканчиваться на `:free`
- Валидация гарантирует, что выбранная модель-судья присутствует в `JUDGE_MODEL`, по умолчанию используется `deepseek-chat`
...
{{"winner": "MODEL_A", "reason": "краткое, но аргументированное обоснование с указанием использованных метрик"}}
или
{{"winner": "MODEL_B", "reason": "краткое, но аргументированное обоснование с указанием использованных метрик"}}
или
{{"winner": "DRAW", "reason": "краткое обоснование, почему решения равнозначны"}}
...
### 3. Добавление моделей-судей

Судьи живут в отдельном словаре `JUDGE_MODEL` в том же файле `src/services/ai_service.py`.  
После добавления записи модель-судья сразу появляется в интерфейсе и доступна в API.
...
### 4. Обновите выпадающий список судей в шаблоне (опционально)

Список судей заполняется автоматически из JavaScript-словаря `JUDGE_MODELS` (файл `index.html`), поэтому после добавления модели в `JUDGE_MODEL` на бэкенде достаточно обновить этот словарь в фронтенде:
...
const JUDGE_MODELS = {
    ...
    // 👇 добавьте нового судью
    "новый-судья": "Отображаемое имя",
};
```
...
- Покрытие включает юнит‑тесты сервисов, интеграционные тесты API и проверку всех веток судьи.
...
- **Судья** по умолчанию — `deepseek-chat`. 
