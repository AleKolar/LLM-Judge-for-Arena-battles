...
- загрузка отчёта на Codecov (опционально)
...
- Автоматический **LLM-судья**:
  - Сравнивает ответы и выносит решение
  - Аргументирует выбор с указанием сильных и слабых сторон
  - При ошибке одной из моделей побеждает рабочая
...
После того как судья вынес вердикт, в интерфейсе появляется кнопка **«Скачать результат»**.

Файл содержит:
- 🏆 Победитель
- ⚖️ Вердикт судьи
- 📊 Статус каждой модели (✅/❌)
- 📝 Полный код и ответы обеих сторон

Формат — **Markdown**, готовый к просмотру и печати.

### 🧠 Системный промпт и задание

Задание, которое получают модели, хранится в `src/prompts/system_prompt.md`. Его легко поменять на любое другое, чтобы устраивать битвы на разные темы.
